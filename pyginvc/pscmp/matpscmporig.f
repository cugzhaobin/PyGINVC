C The gateway routine
      ...
      integer plhs(*), prhs(*)
      integer nlhs, nrhs
c     integer strlen, status
c     character*80 infile, fi
...
C Input #1 must be a string.
c     if (mxIsChar(prhs(1)) .ne. 1) then
c        call mexErrMsgTxt('Input #1 must be a filename.')
c     endif

C Input #1 must be a row vector.
c     if (mxGetM(prhs(1)) .ne. 1) then
c        call mexErrMsgTxt('Input must be a row vector.')
c     endif

C Get the length of input #1 string.
c     strlen = mxGetM(prhs(1))*mxGetN(prhs(1))

C Get the string contents.
c     status = mxGetString(prhs(1), infile, 80)
c     status = mxGetString(prhs(1), fi, 80)

C Check if mxGetString is successful.
c     if (status .ne. 0) then
c        call mexErrMsgTxt('String length must be less than 80.')
c     endif
...
C Call the computational subroutine
     ...
     * Ux,Uy,Uz,Sxx,Syy,Szz,Sxy,Syz,Szx,Tx,Ty,Gd,Gr,X,Y)
