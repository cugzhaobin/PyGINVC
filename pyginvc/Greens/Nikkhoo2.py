...
import numpy as np
from pyginvc.Greens.nikkhoo15lib import nikkhoo15 as nikkhoo
...
class Nikkhoo(object):
    ...
    def __init__(self, flt, data, dict_green):
        ...
        if greenfile == "":
            ...
            self.GenGreens(flt, data, dict_green)
        self.mu      = float(dict_green['modulus'])
        return
...
#               plt.show()
    ...
    def MakeGSAR_Tridisloc(self, unit, node, element, xy, nu, ss, ds, op):
        ...
        for i in range(nelem):
            ...
            # strike slip
            if ss == 1:
                n,e,u = nikkhoo.computedisplacementnikkhoo15(xy[:,1], xy[:,0], np.zeros(len(xy)), 
                                                         Y, X, Z, mu, lam, 1, 0, 0)
                u = np.array([e, n, -u])
